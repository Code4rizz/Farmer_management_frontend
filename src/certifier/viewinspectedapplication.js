...
function ViewInspectedApplication(){
...
const navigate = useNavigate();
  ...
  const statusVariant = {
    ...
    const HandleCertify = (id) => {
     navigate(`/certifiercheck/${id}`);}

useEffect(() => {
    const fetchApplication = async () => {
        try {
            const res = await api.get("/farmer/all_applications");
            setapplications(res.data.inspectedapplications);
            setloading(false);
        } catch (err) {
                    ...
                    <Col md={6} lg={4} key={app._id} className="mb-4">
                        <Card className="h-100 shadow-sm border-1 border-dark">
                              ...
                              <div className="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <div className="mb-3">
                                    ...
                                    <small>{new Date(app.createdAt).toLocaleDateString('en-US', {
                                     ...
                                     <div className="d-flex justify-content-between align-items-center mb-2">
                                    <span className="text-muted fw-bold">Inspected:</span>
                                    <small>{new Date(app.Inspection.idate).toLocaleDateString('en-US', {
                                        year: 'numeric',
                                        month: 'short',
                                        day: 'numeric'
                                    })}</small>
                         ...
                         <h6 className="text-muted mb-2">Note:</h6>
                            <p className="small text-muted mb-0">
                             {app.Inspection.reason
                              ? app.Inspection.reason.slice(0, 100) + (app.Inspection.reason.length > 100 ? "..." : "")
                             : "No Notes available"}
                           </p>
                             </div>
                                </Card.Body>
                                  <Card.Footer className="bg-transparent border-0 d-flex justify-content-center" >
                                        ...
                                        <Button variant="outline-primary" className="fs-6" onClick={()=>HandleCertify(app._id)}>
                                            Certify
                                            </Button>
...
export default ViewInspectedApplication;
